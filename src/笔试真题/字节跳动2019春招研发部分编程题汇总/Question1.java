 ...
 * https://www.nowcoder.com/test/question/42852fd7045c442192fa89404ab42e92?pid=16516564&tid=35940297
 */
...
public class Question1 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <n ; i++) {
            System.out.println(f(new StringBuilder(in.next()),0));
        }
    }

    public static StringBuilder f(StringBuilder s,int index){
        if (index==s.length()) return null;
        if (s.length()>=index+3 && s.charAt(index)==s.charAt(index+1) && s.charAt(index+1)==s.charAt(index+2)){
            s.deleteCharAt(index);
            f(s, index);
        }else if (s.length()>=index+4 && s.charAt(index)==s.charAt(index+1) && s.charAt(index+2)==s.charAt(index+3) &&
